...
class PlnPascabayarConfirmationActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_pln_pascabayar_confirmation)

        val user = intent.getParcelableExtra<GetUserResponse>("dataUser")
        val intent = intent
        val noMeter = intent.getStringExtra("noMeter")
        noMeterTextView.text = noMeter
        val name = intent.getStringExtra("name")
        nameMemberTextView.text = name
        val segmentPower = intent.getStringExtra("segmentPower")
        dayaTextView.text = segmentPower
        val bill = intent.getStringExtra("bill")
        periodTextView.text = bill
    ...
    private fun startPurchase(user: GetUserResponse?) {
        ...
        AndroidNetworking.post("https://risfund.loophole.site/api/purchase/pln-postpaid")
            ...
            .getAsJSONObject(object: JSONObjectRequestListener {
                override fun onResponse(response: JSONObject?) {
                    try {
                        if(response!!.getString("message").equals("Purchase pln postpaid succeeded")) {
                            ...
                            val intent = Intent(this@PlnPascabayarConfirmationActivity, PurchasesSuccessActivity::class.java)
